...
func TestR021_RolloverMetricsNoStaleType(t *testing.T) {
	...
	st := statepkg.NewState("/tmp/does-not-matter")
	...
	st.SetZone(domain, &statepkg.ZoneState{Rollover: &statepkg.RolloverState{Type: "ksk"}})
	...
	st.UpdateZone(domain, func(zs *statepkg.ZoneState) { zs.Rollover = &statepkg.RolloverState{Type: "algorithm"} })
	UpdateZoneMetrics(st)
	...
	st.UpdateZone(domain, func(zs *statepkg.ZoneState) { zs.Rollover = nil })
	UpdateZoneMetrics(st)
