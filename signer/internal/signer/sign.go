...
func (s *Signer) loadKeysForSigning(domain string, keyGen *KeyGenerator, zoneState *statepkg.ZoneState) (*signingKeys, error) {
	...
	if zoneState.Rollover != nil {
		...
		case zoneState.Rollover.Type == "ksk" && (zoneState.Rollover.State == statepkg.KSKRolloverStateDSAddWait || zoneState.Rollover.State == statepkg.KSKRolloverStateDSPropagation):
			...
			if err != nil {
				return nil, fmt.Errorf("loading old KSK %d for rollover signing (if the rollover is complete and the old key is gone, run `sigillum-signer rollover complete %s`): %w", zoneState.Rollover.OldKeyID, domain, err)
			}
		...
		case zoneState.Rollover.Type == "zsk" && zoneState.Rollover.State == statepkg.ZSKRolloverStateSigning:
			...
			if err != nil {
				return nil, fmt.Errorf("loading old ZSK %d for publish during rollover (if the rollover is complete and the old key is gone, run `sigillum-signer rollover complete %s`): %w", zoneState.Rollover.OldKeyID, domain, err)
			}
			keys.dnskeys = append(keys.dnskeys, oldZSK)
		...
		case zoneState.Rollover.Type == "algorithm" && zoneState.Rollover.State != statepkg.AlgoRolloverStateRetiring:
			...
			oldKSK, oldKSKPriv, err := keyGen.LoadKeyPairByID(domain, "ksk", zoneState.Rollover.OldKeyID)
			if err != nil {
				return nil, fmt.Errorf("loading old-algorithm KSK %d for rollover (if the rollover is complete and the old key is gone, run `sigillum-signer rollover complete %s`): %w", zoneState.Rollover.OldKeyID, domain, err)
			}
			keys.dnskeys = append(keys.dnskeys, oldKSK)
			keys.signingKSKs = append(keys.signingKSKs, oldKSK)
			keys.signingKSKPs = append(keys.signingKSKPs, oldKSKPriv)
			...
			if err != nil {
				return nil, fmt.Errorf("loading old-algorithm ZSK %d for rollover (if the rollover is complete and the old key is gone, run `sigillum-signer rollover complete %s`): %w", zoneState.Rollover.OldZSKID, domain, err)
			}
			keys.dnskeys = append(keys.dnskeys, oldZSK)
...
func (k *signingKeys) validateAlgorithms(zoneState *statepkg.ZoneState) error {
	...
	for _, dk := range k.dnskeys[1:] {
		if dk.Algorithm != alg {
			return fmt.Errorf("live keys use different algorithms (%s key tag %d, %s key tag %d) with no algorithm rollover in progress; refusing to publish an algorithm-incomplete zone — use `sigillum-signer rollover algorithm` to change algorithms",
...
func (s *Signer) writeSignedZone(domain, path string, records []dns.RR) error {
	...
	if _, err = fmt.Fprintf(f, "; Generated by sigillum-signer at %s\n", time.Now().UTC().Format(time.RFC3339)); err != nil {
		return fmt.Errorf("writing zone header: %w", err)
	}
...
func (s *Signer) checkRolloverWarnings(domain string, zoneState *statepkg.ZoneState) {
	...
	if zoneState.KSK != nil && now.After(zoneState.KSK.RolloverDue) {
		zoneState.AddWarning("KSK rollover due - run 'sigillum-signer rollover start " + domain + "'")
	}
