...
const (
	// rsaMinBits is the smallest legacy modulus accepted for an imported or
	// loaded key. Go 1.24 raised crypto/rsa's default floor to 1024 bits; the
	// module opts back into the earlier behavior so takeover can restore an
	// existing parent-trusted chain. Keys generated by this signer remain
	// rsaGeneratedBits.
	rsaMinBits = 512
...
func checkRSABits(bits int) error {
	if bits < rsaMinBits {
		return fmt.Errorf("RSA key is %d bits; at least %d bits are required", bits, rsaMinBits)
	}
	if bits > rsaMaxBits {
