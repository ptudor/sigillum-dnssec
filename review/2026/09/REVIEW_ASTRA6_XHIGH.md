...
- **Severity:** High
- **Status:** Confirmed
- **Location:** `signer/internal/signer/rollover.go`, `StartKSKRollover`; `keys.go`, `generateKey`; `sign.go`, `loadKeysForSigning`, `verifySignedZone`; `main.go`, `runRolloverAlgorithm`.
...
- **Severity:** High
- **Status:** Confirmed
- **Location:** `signer/internal/signer/rollover.go`, `StartKSKRollover`, `startZSKRollover`, `StartAlgorithmRollover`; `keys.go`, `SaveKeyFiles`, `RecoverKeyState`; `sign.go`, `loadKeysForSigning`.
...
- **Severity:** High
- **Status:** Confirmed
- **Location:** `signer/main.go`, `verifyNewKSKDSAtParent`, `runRolloverComplete`; `internal/signer/rollover.go`, `CompleteKSKRollover`, `CompleteAlgorithmRollover`.
...
- **Severity:** High
- **Status:** Confirmed
- **Location:** `signer/internal/signer/sign.go`, `SignZone`; `internal/signer/rollover.go`, `handleZSKRolloverState`; `main.go`, `runPostSignHook`; `daemon.go`, `checkAndSignZone`, `signAllZones`.
...
- **Severity:** High
- **Status:** Confirmed
- **Location:** `signer/daemon.go`, `checkAndSignZone`; `internal/signer/sign.go`, `NeedsSign`, `SignAll`, `SignZone`; `main.go`, `runResign`.
...
- **Severity:** High
- **Status:** Confirmed
- **Location:** `signer/main.go`, `runServe`, `loadConfigAndState`; `daemon.go`, `Run`, `validateStartup`; `internal/state/state.go`, `Save`.
...
- **Location:** `validator/internal/dns/query.go`, `parseResponse`; `internal/validator/validator.go`, `queryDSFromParentWithValidation`, `validateZone`, `verifyDSRRSIGSet`; `internal/validator/dnssec.go`, `CollectDSMatchedKeys`.
...
- **Location:** `validator/internal/dns/anchors.go`, `finalizeAnchors`; `internal/validator/dnssec.go`, `VerifyRootTrustAnchor`, `CollectAnchorMatchedKeys`; `internal/validator/validator.go`, root branch of `validateZone`.
...
- **Severity:** High
- **Status:** Confirmed by an isolated executable probe
- **Location:** `validator/internal/validator/validator.go`, `verifyActualRecord`, `verifyWildcard`; `internal/validator/dnssec.go`, `FindRRSIGForType`, `VerifyRRsetRRSIGFromResponseAnyKey`.
...
- **Location:** `validator/internal/validator/nsec.go`, `VerifyNSECDenial`, `verifyNSECNXDOMAIN`, `closestEncloserFromNSEC`.
...
- **Location:** `validator/internal/validator/nsec.go`, `VerifyWildcardDenial`, NSEC branch.
...
- **Location:** `validator/internal/validator/nsec.go`, `verifyNSEC3NXDOMAIN`, `nsec3Matches`, `nsec3Covers`, `VerifyWildcardDenial`; `validator.go`, `verifyDSAbsence`, `recordValidationVerdict`.
...
- **Location:** `validator/internal/validator/nsec.go`, `VerifyWildcardDenial`, `groupNSEC3ByParams`; `validator.go`, `verifyDSAbsence`.
...
- **Location:** `validator/internal/validator/nsec.go`, `verifyNSECNODATA`, `verifyNSEC3NODATA`; `validator.go`, `verifyActualRecord`; `internal/dns/query.go`, `parseResponse`.
...
- **Location:** `validator/internal/validator/validator.go`, `verifyActualRecord`, `checkAndFollowCNAME`, `validateWithCache`; `internal/dns/query.go`, `parseResponse`.
...
- **Location:** `validator/internal/validator/dnssec.go`, `verifyDNSKEYRRSIGOne`, `VerifyRRsetRRSIGFromResponse`, `VerifyDenialRRSIGFromResponse`, `reconstructDNSKEY`; `internal/dns/types.go`, `DNSKEYRecord`/`RRSIGRecord`.
...
- **Location:** `validator/internal/validator/validator.go`, `verifyActualRecord`, `validateZone`; `dnssec.go`, `ValidateChainLink`; `nsec.go`, `VerifyNSECDenialWithRRSIG`, `VerifyNSEC3DenialWithRRSIG`.
...
- **Severity:** Medium
- **Status:** Confirmed
- **Location:** `validator/internal/validator/validator.go`, `ValidateMultipleServers`, `compareDNSKEYSets`, `queryLeafAllServers`, `leafFingerprint`; `static/app.js`, `showZoneDetails`.
...
- **Severity:** Medium
- **Status:** Confirmed
- **Location:** `validator/internal/dns/resolver.go`, `exchangeRecursive`, `ResolveNS`, `ResolveAddresses`, `CheckZoneCut`, `DiscoverZoneCuts`; `internal/validator/validator.go`, `validateZone`.
...
- **Severity:** Medium
- **Status:** Confirmed
- **Location:** `validator/server.go`, `nonSSEWriteTimeout`, `registerRoutes`, `withWriteDeadline`; `handlers.go`, `HandleValidateJSON`.
...
- **Severity:** Medium
- **Status:** Confirmed
- **Location:** `validator/internal/metrics/metrics.go`, `promAPIRequests`, `RecordAPIRequest`; `handlers.go`, all method-rejection paths.
...
- **Severity:** Medium
- **Status:** Confirmed
- **Location:** `validator/internal/validator/validator.go`, early Bogus returns in `validateWithCache`, leaf validation/cache updates; `static/app.js`, `addZoneCard`, `completeValidation`, error/complete event handlers.
...
- **Location:** `signer/internal/signer/rollover.go:416–456`, `BackupKey`; `internal/signer/keys.go:279–350`, `backupExistingKeyFiles`; `internal/fsutil/fsutil.go`, `CopyFile`.
...
- **Severity:** High
- **Status:** Confirmed
- **Location:** `signer/main.go:1398–1558`, `runImport`, `loadBindKeyPair`; `internal/signer/keys.go`, `SaveKeyFiles`.
...
- **Severity:** High
- **Status:** Confirmed by executable probe
- **Location:** `signer/internal/state/state.go:189–229`, `ReloadFromDisk`, `rolloverEqual`; `daemon.go`, `signAllZones`; `registrar_cli.go`, `RunRegistrarPush`, `recordRegistrarWarning`, `clearRegistrarStickyWarnings`; `main.go`, `runRemove`.
...
- **Severity:** High
- **Status:** Confirmed
- **Location:** `signer/daemon.go`, `signAllZones` (both `ReloadFromDisk` calls); `internal/state/state.go`, `ReloadFromDisk`, `Save`.
...
- **Severity:** High
- **Status:** Confirmed by executable probe
- **Location:** `signer/internal/signer/sign.go:209–222`, `SignZone`; `internal/signer/rollover.go`, `dnskeyTTLFloor`, `zskRetireFloor`; `internal/state/state.go`, published TTL fields.
...
- **Severity:** High
- **Status:** Confirmed by executable probe
- **Location:** `signer/internal/signer/sign.go`, RRset grouping in `signRecordsWithKeys` and `verifySignedZone`, NSEC/NSEC3 owner collection and delegation helpers.
...
- **Severity:** Medium
- **Status:** Confirmed by executable probe
- **Location:** `signer/internal/signer/sign.go`, `validateZoneRecords`, `ValidateZoneFile`, `SignZone`, `verifySignedZone`.
...
- **Severity:** Medium
- **Status:** Confirmed by executable probe
- **Location:** `signer/internal/signer/keys.go`, `VerifyKeyPairCorrespondence`; `main.go`, `loadBindKeyPair`; `internal/signer/crypto.go`.
...
- **Location:** `signer/internal/registrar/registrar_dynadot.go:549–588`, `ReplaceDS`; `registrar_cli.go`, emergency warning handling.
...
- **Severity:** Medium
- **Status:** Confirmed
- **Location:** `signer/internal/validate/validate.go`, `evalRRSIGCover`, `checkRRSIGPresent`, `checkDNSKEYVisible`, `computeOverall`, `bogusWhenDSPresent`; `dnssecstatus/index.html`, `isServfail`.
...
- **Severity:** Medium
- **Status:** Confirmed
- **Location:** `signer/daemon.go`, `checkAndSignZone`, `checkAndSignZoneSafe`; `health.go`, `healthHandler`, `healthzHandler`; `status.go`, `buildStatusOutput`; `internal/metrics/metrics.go`, `UpdateZoneMetrics`.
...
- **Location:** `signer/hooks.go`, `executeHook`, `executeBatchHook`, `executeHookSync`; `daemon.go`, hook wait/shutdown handling.
...
- **Severity:** Medium
- **Status:** Confirmed
- **Location:** `signer/internal/signer/sign.go`, `NeedsSign`; `daemon.go`, reload and signing-cycle change detection.
...
- **Severity:** Medium
- **Status:** Confirmed
- **Location:** `signer/internal/state/state.go`, `LoadState`, `ReloadFromDisk`, `SetZone`, `SnapshotZones`; `internal/signer/sign.go`, `loadKeysForSigning`.
...
- **Location:** `signer/heartbeat.go:43–66`, `NewHeartbeatClient`, `Send` (111–136); `internal/config/config.go:506–508`.
...
- **Location:** `signer/internal/config/config.go:618–736`, `AddZoneToConfigFile`, `RemoveZoneFromConfigFile`, table-header helpers.
...
- **Location:** `signer/scripts/dynadot-probe.sh:137–141`, `call`; reference implementation `internal/registrar/registrar_dynadot.go`, request signature construction.
...
- **Location:** `signer/daemon.go:180`, `Reload`; `daemon.go:287`, `runSigningLoop`; `Run` startup and `main.go`, `runServe` signal handling.
...
- **Location:** `validator/internal/dns/anchors.go:107–147`, `finalizeAnchors`; `LoadAnchorsWithFallback` (233–246); `GetActiveAnchors` (250–276); `anchors_store.go:28–39`, `Load`.
...
- **Location:** `signer/internal/signer/sign.go:106–121`, `SignZone`, `parseZoneFile`, `NeedsSign`; `daemon.go`, `checkAndSignZone`.
...
- **Severity:** Medium
- **Status:** Confirmed
- **Location:** `signer/main.go:631–636`, `runSign`, `runPostSignHook`; `hooks.go`, `executeHookSync`, `executeBatchHook`; `README.md:297–314`.
...
- **Location:** `signer/internal/fsutil/fsutil.go:35–89`, `InitOwnershipTarget`, `ChownToTarget`; `WriteFileAtomicOwned` (111–148); CLI directory setup.
...
- **Location:** `validator/QUICKSTART-FREEBSD.md:9–16,66–73,115`; validator `Makefile:43–46`; `signer/QUICKSTART.md:25–28,54,150–154,209–210`; signer `README.md:125–126`.
...
- **Location:** `validator/docs/RFC_FIXES.md`, multi-algorithm and denial-proof prescriptions; `docs/RFC_COMPLIANCE.md`, algorithm support/status claims; `signer/RFC_COMPLIANCE_REVIEW.md:168–183`; signer `CLAUDE.md`, rollover and future-scope sections.
...
- **Location:** `signer/internal/signer/keys.go:380–391`, `formatPrivateKey`, key generation; `README.md`, key import/recovery documentation.
...
- **Severity:** Medium
- **Status:** Confirmed
- **Location:** `signer/main.go:618–649`, `runSign`; `runResign` (676–691); single-zone rollover signing handlers; `internal/signer/sign.go:77–83,203–227`.
...
- **Severity:** Medium
- **Status:** Confirmed
- **Location:** `signer/internal/fsutil/fsutil.go:141–148,199–230`, `WriteFileAtomicOwned`, `WriteConfigFileAtomic`, `SyncDir`; key backup/activation and state/config callers.
...
- **Location:** `signer/internal/config/config_validation_test.go:153–159`, `TestLoadConfig_TestdataStillLoads`; `internal/registrar/registrar_test.go:102–110`, `TestDynadotSign_Deterministic`; `internal/validate/validate_test.go:178–230`, `TestDNSKEYCheck`; validator `internal/validator/nsec_test.go`, `TestComputeNSEC3Hash`.
...
- **Location:** `signer/internal/registrar/registrar_ratelimit.go:9–85`, `tierDelay`, `slidingLimiter.gate`; `registrar_dynadot.go`, shared limiter and request calls.
...
- **Location:** `validator/internal/validator/dnssec.go:121–135`, `VerifyRRsetRRSIGFromResponseAnyKey`; `VerifyDNSKEYRRSIGByKeys`, `verifyDNSKEYRRSIGOne`, `VerifyDenialRRSIGFromResponse`; validation timeout/concurrency handling in `handlers.go`.
...
- **Location:** `signer/internal/signer/sign.go:704–737`, `findDelegationPoints`, `delegationInfo.isOccluded`; `generateNSECChain`, `generateNSEC3Chain`, `signRecordsWithKeys`, `verifySignedZone`; daemon signing lock.
...
- **Location:** `validator/internal/dns/resolver.go:56–129`, `ResolveNS`, `ResolveAddresses`, `ResolveNSWithAddresses`; `query.go:41–79`, `QueryWithRecursion`; authoritative query loops in `internal/validator/validator.go`; public JSON/SSE handlers.
...
- **Location:** `signer/scripts/dynadot-probe.sh:79–81`, `hmac_b64`; request construction at 164–177.
...
- **Location:** `signer/internal/signer/sign.go:817–912`, `verifySignedZone`; `verifyNSECChainClosure`, `verifyNSEC3ChainClosure`, `walkDenialChain`.
